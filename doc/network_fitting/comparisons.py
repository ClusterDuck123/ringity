...
def save_values_to_json(folder, filename, density, avg_degree, N, parameters, fitter_c, fitter_w):
    """
...
def extract_and_save_info(folder, filename, G, parameters):
    ...
    save_values_to_json(folder, filename, density, avg_degree, N, parametenotebooks/network_fitting/comparisons.pyrs, fitter.c, fitter.w)
...
for name, G_true in networks.items():
    ...
    for _ in range(100):
        ...
        try:
            ...
            G,positions = rg.network_model(N,density=density,beta=beta,r=r,return_positions=True)
