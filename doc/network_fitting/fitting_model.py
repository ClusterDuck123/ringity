...
def truncated_exponential(theta, scale, decay_rate, truncation_point):
    ...
    """
    return scale * decay_rate * np.exp(decay_rate * np.mod(-(theta - truncation_point), 2*np.pi))
...
class NetworkHomophilyAnalyzer:
    ...
    def plot_distance_histogram(self, color='k', n_bins=50):
        ...
        for node_i in self.nodelist:
            for node_j in self.graph.neighbors(node_i):
                dist = self._calculate_angular_distance(
                    self.node_positions[node_j], 
                    self.node_positions[node_i]
                )
        ...
        self.total_counts, bins, _ = ax.hist(
            all_distances, bins=n_bins, color="#AAAAAA", 
            density=False, label="All pairs", alpha=0.7
        )
        self.neighbor_counts, _, _ = ax.hist(
            connected_distances, bins=bins, color=color, 
    ...
    def _create_embedding_sections(self, n_embedding_sections):
        ...
        self.embedding_section_boundaries = np.linspace(-np.pi,np.pi, n_embedding_sections+1)
        
    def _get_section_index(self, x):
            
        section_index_incremented = np.digitize(np.mod(x, 2*np.pi) - np.pi, self.embedding_section_boundaries)
        assert section_index_incremented > 0
        return section_index_incremented - 1 
        
    def calculate_distance_histogram_by_section(self,
                                           color='k',
                                           n_bins=50,
        ...
        self.connection_probabilities_split          = [None for _ in range(n_embedding_sections)]
        ...
        for section_index in range(n_embedding_sections):
            ...
            self.total_counts_split[section_index], bins, _ = ax.hist(
                all_distances[section_index], bins=n_bins, color="#AAAAAA", 
                density=False, label="All pairs", alpha=0.7
            )
            self.neighbor_counts_split[section_index], _, _ = ax.hist(
                connected_distances[section_index], bins=bins, color=color, 
                label="Connected pairs"
            )
            ax.set_xlabel("Angular Distance", fontsize=12)
            ax.set_ylabel("Count", fontsize=12)
            ax.set_title(f"""Distribution of Distances: All Pairs vs Connected Pairs\n
            ...
            self.connection_probabilities_split[section_index] = self.neighbor_counts_split[section_index] / self.total_counts_split[section_index]
    ...
    def fit_connection_probability_model(self):
        ...
        if self.connection_probabilities is None:
            raise ValueError("Must call plot_distance_histogram() first to generate data")
    ...
    def plot_connection_probability_by_section(self, color='k'):
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        colors = [np.array([1.0, 1.0, 1.0])*i/self.n_embedding_sections for i in range(self.n_embedding_sections)]
        m_list = []
        ...
        ax.set_ylabel("Connection Probability", fontsize=12)
        ax.set_title("Connection Probability vs Distance")
        ax.legend(m_list,labels=[f"{self.embedding_section_boundaries[section_index]:.3f} to {self.embedding_section_boundaries[section_index+1]:.3f}"
    ...
    def _plot_edge_positions_with_section(self):
        ...
        circular_means = [self._calculate_angular_mean(all_edge_positions[i,0],all_edge_positions[i,1]) for i in range(all_edge_positions.shape[0])]
        
        color = [self._get_section_index(circular_mean) for circular_mean in circular_means]
        # Plot the points
        point_size = max(1, 50 / np.sqrt(len(self.graph.edges())))
        ax.scatter(
            all_edge_positions[:, 0], 
    ...
    def get_homophily_summary(self):
        ...
        return {
            ...
            'homophily_strength': 'Strong' if self.interaction_width < np.pi/4 else 
                                 'Moderate' if self.interaction_width < np.pi/2 else 'Weak'
        }
...
def analyze_position_distribution(n_nodes, node_positions, n_bins=None, color='k'):
    ...
    counts, bins, _ = ax.hist(positions, bins=n_bins, color=color, 
    ...
    fitted_density = truncated_exponential(theta_space, scale, decay_rate, truncation_point)
    ax.plot(theta_space, fitted_density, color="#777777", linewidth=3, 
            label=f"Fitted Truncated Exponential", linestyle='--')
...
def run_analysis(
    ...
    if randomization == "configuration":
        ...
        print(f"randomization method {randomization} not found,\ndefaulting to no randomization...")
        randomization = False
    ...
    if load_embedding:
        ...
        graph_position_finder.make_circular_spring_embedding(iterations=embedding_iterations)
    ...
    if append_summary:
        ...
        info = pd.DataFrame([[network_file, embedding_file, embedding_iterations, randomization, analyzer.interaction_width, analyzer.max_probability]])
        info.columns = ["network_file", "embedding_file", "embedding_iterations", "randomization", "analyzer_interaction_width", "analyzer_max_probability"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--network-file",         type=str,  default="data/empirical_networks/immune.gml")
