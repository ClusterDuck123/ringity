...
def estimate_std(array_of_values, window_size=30):
    ...
    return np.sqrt(correction * (smooth_sum_square/smooth_count - 
                                (smooth_sum_value/smooth_count)**2))
...
class PositionGraph:
    ...
    def make_circular_spring_embedding(self, verbose=False, k=None, iterations = 300,pos=None):
        """
        ...
        if pos == None:
            ...
            pos = 2 * (np.asarray(np.random.random((self.n_nodes, dim)), dtype=A.dtype) - 0.5)
    ...
    def positions_to_uniform_embedding(self):
        ...
        self.unadjusted_embedding_dict = dict(zip(self.nodelist, 
                                                 self.unadjusted_embedding_array))
        
        self.nodelist_sorted_unadjusted = sorted(self.nodelist, 
                                        key=self.unadjusted_embedding_dict.get)
    ...
    def smooth_neighborhood_widths(self,window_size=None):
        ...
        for node in self.nodelist_sorted_unadjusted:
            ...
            neighbor_positions = [self.unadjusted_embedding_dict[neighbor] 
            ...
            centered_positions = [np.mod(pos - circ_mean + np.pi, 2*np.pi) 
                                for pos in neighbor_positions]
            
            neighborhood_samples.append(np.array(centered_positions))

        # Smooth the neighborhood width estimates
        self.stds_smoothed = estimate_std(neighborhood_samples, 
    ...
    def recenter_and_reorient_calcs(self):
        ...
        tripled_stds = (list(self.stds_smoothed) + 
                       list(self.stds_smoothed) + 
                       list(self.stds_smoothed))
        
        self.look_for_discontinuity_std = np.convolve(tripled_stds, edge_filter, mode='same')

        # Find position of maximum change (in middle third to avoid boundary effects)
        unadjusted_embedding_array = np.array([self.unadjusted_embedding_dict[node] 
                                              for node in self.nodelist_sorted_unadjusted])
        
        max_change_idx = np.argmax(np.abs(self.look_for_discontinuity_std[n_nodes:2*n_nodes]))
        self.embedding_cutoff = unadjusted_embedding_array[max_change_idx]
        self.sign_change = np.sign(self.look_for_discontinuity_std[n_nodes:2*n_nodes][max_change_idx])
    ...
    def reparametrize(self, stds_smoothed):
        ...
        ipdf_smooth_dict = dict(zip(self.nodelist_sorted_unadjusted, self.stds_smoothed))
        self.nodelist_sorted = sorted(self.nodelist, key=self.embedding_dict.get)
        
        # Use reciprocal of width as density estimate
        self.ipdf_smooth_adjusted = np.array([1/ipdf_smooth_dict[node] 
                                            for node in self.nodelist_sorted])
    ...
    def reconstruct_positions(self):
        ...
        embedding_positions = np.array([self.embedding_dict[node] 
                                      for node in self.nodelist_sorted])
        
        # Apply inverse CDF transformation
        u_midpoints, cumulative_icdf = reconstruct_icdf(
            embedding_positions / (2*np.pi), 
            self.ipdf_smooth_adjusted
        )
...
if __name__ == "__main__":
    ...
    self.make_circular_spring_embedding(verbose=True,
    ...
    if args.input_true_positions != "none":
        ...
        ax.scatter(true_positions_df.reindex(positions_df.index).values,
                   positions_df.values,
                   rasterized=True,
                   c="k",
                   )
