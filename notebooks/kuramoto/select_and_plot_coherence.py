...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-csv", type=str, default="data/search_for_diverse_dynamics.csv", help="The input folder")
    parser.add_argument("--input-folder", type=str, default="data/parameter_array/", help="The input folder")
    parser.add_argument("--output-folder", type=str, default="figures/reruns/", help="The output folder")
    ...
    for _,v in df.sort_values("terminal_std").iloc[-n:].iterrows():
        ...
        fig.savefig(output_folder / f"asynch_{i}.png", 
                    dpi= 300,
                    transparent=True, 
                    bbox_inches="tight"
                    )
    ...
    for _,v in df.sort_values("terminal_mean").iloc[-n:].iterrows():
        ...
        ax.set_xlabel("time")
        ax.set_ylabel("phase coherence")
        fig.savefig(output_folder / f"coherent_{i}.png", 
                    dpi= 300,
                    transparent=True, 
                    bbox_inches="tight"
                    )
    ...
    for _,v in df.sort_values("terminal_mean").iloc[:n].iterrows():
        ...
        ax.set_xlabel("time")
        ax.set_ylabel("phase coherence")
        fig.savefig(output_folder / f"twisted_{i}.png", 
                    dpi= 300,
                    transparent=True, 
                    bbox_inches="tight"
                    )
