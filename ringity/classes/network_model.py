...
def slope(rho, rate, a):
    ...
    if rho <= mu_S:
        ...
        def integral(k):
            ...
            term2 = (k * np.sinh((a * PI * rate) / k) *
                     np.sinh(((a + k - 2 * a * k) * PI * rate) / k)) / (a * PI * rate)
            return term1 - term2
        return scipy.optimize.newton(
            func=lambda k: const *
            integral(k) + (1 - cdf_similarity(1 / k, rate, a)) - rho,
            x0=rho / mu_S)
...
class RandomPositions:
    def __init__(self, N,
                 distribution = 'wrapped_exponential',
                 parms = {},
                 random_state = None):
        """
        ...
        if isinstance(distribution, ss._distn_infrastructure.rv_frozen):
            self.positions = distribution.rvs(size = N,
                                              random_state = random_state)
        elif distribution == 'wrapped_exponential':
