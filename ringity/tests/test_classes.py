...
class TestDgmPt(unittest.TestCase):
    def setUp(self):
        ...
        self.dgm_pt = ringity.classes.random_DgmPt()
...
class TestDgm(unittest.TestCase):
    def setUp(self):
        self.number = random.random()
        self.signal = ringity.classes.DgmPt(0, self.number)
        self.noise  = ringity.classes.DgmPt(0, 0)

    def test_score1(self):
        dgm = ringity.classes.Dgm(self.signal for _ in range(100))
    ...
    def test_birth_and_death_extractions(self):
        dgm = ringity.classes.random_Dgm(length=100)
