...
def infer_response_parameter(density, coupling, rate):
    if density > coupling:
        raise AttributeError(
            f"Density can't be higher than coupling. "
    ...
    if np.isclose(rate, 0):
        ...
        def response_equation(r):
            return governing_equation(
                response=r, density=density, coupling=coupling, rate=rate
            )
...
def infer_rate_parameter(response, density, coupling):
    def rate_equation(lamb):
        return governing_equation(
            response=response, density=density, coupling=coupling, rate=lamb
        )
...
def parse_response_parameter(r=None, a=None, alpha=None, response=None):
    ...
    if nb_parms == 0:
        ...
        raise ValueError(
            f"Please provide only one response parameter! "
        ...
        warning_message = (
            "Using the parameter `a` for the response length "
        ...
        warning_message = (
            "Using the parameter `alpha` for the response length "
            "is depricated. Please use the paramter `r` instead!"
        )
        warn(warning_message, DeprecationWarning, stacklevel=2)
        response = alpha
...
def parse_rate_parameter(rate, beta):
    if beta is None and rate is None:
        raise ValueError(
            f"Please provide a distribution parameter: " f"beta = {beta}, rate = {rate}"
        )

    elif beta is not None and rate is not None:
        raise ValueError(
            f"Conflicting distribution parameters given: "
...
def parse_coupling_parameter(c=None, K=None, coupling=None):
    ...
    if nb_parms == 0:
        ...
        raise ValueError(
            f"Please provide only one coupling parameter! "
            f"r = {r}, a = {a}, alpha = {alpha}, "
            f"response = {response}"
        )
        ...
        warning_message = (
            "Using the parameter `K` for the coupling strength "
...
def parse_density_parameter(rho=None, density=None):
...
def parse_canonical_parameters(params):
    rate = parse_rate_parameter(rate=params["rate"], beta=params["beta"])
    response = parse_response_parameter(
        r=params["r"], a=params["a"], alpha=params["alpha"], response=params["response"]
    )
    coupling = parse_coupling_parameter(
        c=params["c"], K=params["K"], coupling=params["coupling"]
    )
    density = parse_density_parameter(rho=params["rho"], density=params["density"])
    model_params = {
        "rate": rate,
        "response": response,
        "coupling": coupling,
