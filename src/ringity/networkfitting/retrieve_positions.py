...
def estimate_std(array_of_values,window_size = 30):
    ...
    return np.sqrt(correction*(smooth_sum_square/smooth_count - (smooth_sum_value/smooth_count)**2))
...
class PositionGraph:
    ...
    def make_circular_spring_embedding(self, verbose=False, k=None):
        ...
        self.unadjusted_embedding_array =(2*np.pi*scipy.stats.rankdata(unadjusted_embedding_array)/
                                          len(unadjusted_embedding_array))
        
        self.unadjusted_embedding_dict = dict(zip(self.nodelist, self.unadjusted_embedding_array))
    ...
    def smooth_neighborhood_widths(self):
        ...
        self.nodelist_sorted_unadjusted = sorted(self.nodelist, key=self.unadjusted_embedding_dict.get)
        ...
        for i in self.nodelist_sorted_unadjusted:
            ...
            rank_neighbors = np.array([self.unadjusted_embedding_dict[j] for j in self.G.neighbors(i)]+[self.unadjusted_embedding_dict[i]])
            ...
            rank_neighbors_center_pi = np.array([np.mod(np.array(x)-circ_mean+ np.pi, 2*np.pi) for x in rank_neighbors])
    ...
    def recenter_and_reorient_calcs(self):
        ...
        look_for_discontinuity_std = np.convolve(list(stds_smoothed)+list(stds_smoothed)+list(stds_smoothed),
                             edge_filter,
                             mode='same')
        ...
        unadjusted_embedding_array = np.array([self.unadjusted_embedding_dict[i] 
                                               for i in self.nodelist_sorted_unadjusted])
    ...
    def reparametrize(self, stds_smoothed):
        ...
        self.ipdf_smooth_adjusted = np.array([1/ipdf_smooth_dict[i] for i in self.nodelist_sorted])
    ...
    def recenter_and_reorient(self, reparametrize=True,calculate=True):
        ...
        self.embedding_dict = {i:np.mod(self.sign_change*(v-self.embedding_cutoff), 2*np.pi) 
    ...
    def reconstruct_positions(self):
        ...
        embedding_positions = np.array([self.embedding_dict[j] for j in self.nodelist_sorted])
        u_midpoints, cumulative_icdf = reconstruct_icdf(embedding_positions / (2*np.pi),self.ipdf_smooth_adjusted)
