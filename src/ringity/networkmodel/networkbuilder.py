...
class NetworkBuilder:
    ...
    def model(self, value):
        if hasattr(self, "model"):
            raise AttributeError(
                f"Trying to set conflicting models " f"{value} != {self.model}"
            )
        self._model = value
    ...
    def distances(self, value):
        ...
        if value.shape == (self.pwN,):
            ...
            raise ValueError(
                f"Trying to set invalid distances!" f"{value.shape, self.pwN}"
            )
    ...
    def _build_general_model(self):
        assert self.rate > 0
        self.set_distribution(distn_arg="exponential", scale=1 / self.rate)
        self.instantiate_positions()
        self.calculate_distances(metric="euclidean", circular=True)
        self.calculate_similarities(r=self.response, sim_func="box_cosine")
        self.calculate_probabilities(
            prob_func="linear", slope=self.coupling, intercept=0
        )
    ...
    def _build_zero_distance_model(self):
        ...
        if not np.isclose(self.density, self.coupling):
            raise ValueError(
                f"Conflicting values for zero-distance model found: "
    ...
    def calculate_similarities(self, sim_func="box_cosine", r=None):
        ...
        self.similarities = sim_func(
            squareform(self.distances), box_length=2 * np.pi * self.response
        )
    ...
    def calculate_probabilities(self, prob_func="linear", slope=None, intercept=None):
        ...
        self.probabilities = prob_func(
            self._similarities, slope=slope, intercept=intercept
        )
