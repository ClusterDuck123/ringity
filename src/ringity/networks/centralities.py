...
def current_flow(G, inplace=False, new_weight_name=None, verbose=False):
...
def net_flow(G, inplace=False, new_weight_name=None, verbose=False):
    ...
    row_idx, col_idx = map(
        np.array, zip(*((i, j) for (i, j) in zip(A_coo.row, A_coo.col) if i < j))
    )

    edge_values = _current_flow_loop(C, row_idx, col_idx)
    edge_dict = {
        (node_label[i], node_label[j]): edge_values
