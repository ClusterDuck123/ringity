...
def distance_to_knn_graph(
    D, n_neighbors=None, keep_weights=False, new_weight_name="distance", verbose=False
):
...
def distance_to_sublevel_graph(
    D,
    er_fc_th=10,
    rel_dist_th=None,
    abs_dist_th=None,
...
def _get_k_smallest_value_by_col(D, k):
    k_smallest_values = np.apply_along_axis(
        partial(_get_k_smallest_value_in_vec, k=k), axis=0, arr=D
    )
