...
def json_to_dictoflist(fname, lambda_key=lambda x: x, new=False):
...
def json_to_dictofdictoflist(
    fname, lambda_key1=lambda x: x, lambda_key2=lambda x: x, new=False
):
        ...
        with open(fname, "r") as f:
            ...
            for jkey1 in json_dict:
                ...
                for jkey2 in json_dict[dkey1]:
                    ...
                    data_dict[dkey1][dkey2] = list(
                        map(PDiagram, json_dict[jkey1][jkey2])
                    )
    return data_dict
