...
def plot_nx(
    ...
    **kwargs,
    ...
    if library == "plotly":
        if dim == 2:
            return plot_nx_plotly_2d(
                G, pos=pos, hoverinfo=hoverinfo, node_color=node_color
            )
        elif dim == 3:
            return plot_nx_plotly_3d(
                G, pos=pos, hoverinfo=hoverinfo, node_color=node_color
            )
...
def plot_dgm(dgm, ax=None, return_artist_obj=False, **kwargs):
    ...
    ax.plot([0, d], [0, d], color=DARK_CEMM_COL1, linewidth=1, linestyle="dashed")
...
def plot_degree_distribution(G, ax=None, figsize=None, return_artist_obj=False):
    ...
    ax.bar(degs, cnts, width=1, color=CEMM_COL2, linewidth=0.75, edgecolor=CEMM_COL4)
