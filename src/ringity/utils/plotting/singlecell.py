...
def plot_dimreduction_from_anndata(
    adata,
    var_names=None,
    obs_names=None,
    color=None,
    ...
    plot_data = pd.concat([plot_data, adata.obs], axis=1)

    if plotter == "plotly":
        if dim == 2:
            fig = px.scatter(data_frame=plot_data, x="UMAP1", y="UMAP2", color=color)
        if dim == 3:
            fig = px.scatter_3d(
                data_frame=plot_data, x="UMAP1", y="UMAP2", z="UMAP3", color=color
            )
