...
class TestSyntheticExamples(unittest.TestCase):
    ...
    def test_score_of_zero1(self):
        ...
        sequence = (
            self.noise + multiple_signals + self.noise + multiple_signals + self.noise
        )
        self.assertAlmostEqual(ring_score_from_sequence(sequence), 0, places=8)
    ...
    def test_score_of_one(self):
        ...
        self.assertAlmostEqual(ring_score_from_sequence(sequence), 1, places=8)

    def test_score_of_half(self):
        sequence = self.noise + [self.signal] + self.noise + [self.signal] + self.noise
        self.assertAlmostEqual(ring_score_from_sequence(sequence), 0.5, places=8)
