...
INF_RING_SCORES = {geometric_ring_score, amplitude_ring_score, entropy_ring_score}
...
class TestFiniteEdgeCases(unittest.TestCase):
    ...
    def test_empty_seq(self):
        ...
        for ring_score in ALL_RING_SCORES:
            ...
            self.assertAlmostEqual(ring_score(seq, **kwargs), 0.0, msg=msg)
    ...
    def test_noiseless_seq(self):
        ...
        for ring_score in ALL_RING_SCORES:
            ...
            if "nb_pers" in inspect.signature(ring_score).parameters:
                kwargs["nb_pers"] = self.rand_nb_pers
            msg = _generate_message(seq, ring_score, self.rand_nb_pers, self.rand_base)
            self.assertAlmostEqual(ring_score(seq, **kwargs), 1.0, msg=msg)
    ...
    def test_finiteness(self):
        for ring_score in ALL_RING_SCORES - {gap_ring_score}:
            msg = _generate_message(
                self.rand_seq, ring_score, self.rand_nb_pers, self.rand_base
            )
            kwargs = {}
            if "base" in inspect.signature(ring_score).parameters:
                kwargs["base"] = self.rand_base

            score1 = ring_score(self.rand_seq, nb_pers=self.rand_nb_pers, **kwargs)
            score2 = ring_score(
                list(self.rand_seq) + [min(self.rand_seq)],
...
class TestInfiniteCases(unittest.TestCase):
    ...
    def test_min_score_of_zero(self):
        ...
        for ring_score in INF_RING_SCORES:
            ...
            if "nb_pers" in inspect.signature(ring_score).parameters:
                kwargs["nb_pers"] = np.inf
            msg = _generate_message(seq, ring_score, np.inf, self.rand_base)
            self.assertAlmostEqual(ring_score(seq, **kwargs), 0.0, msg=msg, delta=delta)
    ...
    def test_infiniteness(self):
        ...
        for ring_score in INF_RING_SCORES - {entropy_ring_score}:
            ...
            score1 = ring_score(seq, nb_pers=np.inf, **kwargs)
            score2 = ring_score(
                list(seq) + [0] * self.rand_zero_pads, nb_pers=np.inf, **kwargs
            )
...
class TestTwoPersistenceIdentities(unittest.TestCase):
    def setUp(self):
        pseq_list = [
            np.random.uniform(size=N) for _ in range(2**2) for N in range(2, 2**2)
        ]
    ...
    def test_amp_identities(self):
        test_scores1 = [
            (p0 - p1) / (p0 + p1) for (p0, p1) in zip(self.p0_list, self.p1_list)
        ]
    ...
    def test_ent_identities(self):
        ...
        test_scores1 = [
            p1_over_p0_identity(p0 / p1) for (p0, p1) in zip(self.p0_list, self.p1_list)
        ]
